<?php
    ...
    $sql = "SELECT * FROM clothes INNER JOIN example on clothes.name = example.name";

    $sortby = isset($_GET['sort']) ? $_GET['sort'] : 'name';
    if($sortby == 'name') {
        $sql .= " order by clothes.name";
    }
    elseif($sortby == 'color') {
        $sql .= " order by color";
    }
    ...
    echo "<br><br> <a href='example_insert.php' id='insert'>신발 추가</a> <br><br>";

    echo "<div class='search'>";
    echo "<form action='example_search.php' method='get'>";
    ...
    echo "<tr>";
    echo "<th><a href='example_select.php?sort=name'>상품명</th> <th><a href='example_select.php?sort=color'>색상</th> <th><a href='example_select.php?sort=price'>가격</th>";
    echo "<th>사이즈</th> <th>볼넓이</th> <th>총높이</th> <th>굽높이</th>";
    echo "<th><a href='example_select.php?sort=date'>구매날짜</th> <th><a href='example_select.php?sort=preference'>선호도</th> <th>수정</th> <th>삭제</th>";
    echo "</tr>";

    while($row = mysqli_fetch_array($ret)) {
        echo "<tr>";
        echo "<td>", $row['clothes.name'], "</td>";
        ...
        echo "<TD id='update'>", "<A HREF='example_update.php?name=", $row['name'], "'>수정</A></TD>";
        echo "<TD id='delete'>", "<A HREF='example_delete.php?name=", $row['name'], "'>삭제</A></TD>";
    }
